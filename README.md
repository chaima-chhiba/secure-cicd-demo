...
Scans the entire git history using \etch-depth: 0\ for accidentally committed credentials, API keys, or tokens. Runs first — if secrets are found, nothing else executes.
...
Queries the NIST National Vulnerability Database against every package in \equirements.txt\. Pipeline fails if any dependency scores 9.0 or above on CVSSv3.

### 4. Trivy — Container Scanning
Scans the built Docker image for OS-level CVEs. Configured to only fail on CRITICAL vulnerabilities that have a patch available. Results uploaded to the GitHub Security tab in SARIF format. Python package CVEs are handled separately by OWASP.
...
- Non-root user — container runs as \ppuser\, not root
